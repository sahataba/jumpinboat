...
const formatPricingSummary = (boat: BoatListingSummary) => {
  const base = `${boat.route.pricing.basePricePerTrip.amount} ${boat.route.pricing.basePricePerTrip.currency} for the trip`;
  ...
  if (!boat.route.pricing.hasUniformPerStopPricing) {
    ...
    return `${base}, from ${Math.min(...customPrices)} ${boat.route.pricing.basePricePerTrip.currency} for an extra stop`;
  }

  return `${base}, ${boat.route.pricing.uniformPricePerStop?.amount ?? 0} ${boat.route.pricing.basePricePerTrip.currency} for an extra stop`;
};
  ...
  >
    ...
    <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <p className="mt-1 text-sm text-slate-500">Room for people and bags</p>
      </div>
      <div className="rounded-2xl bg-slate-50 p-4">
    ...
    <main className="jb-page">
      <section className="jb-section">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              <h1 className="max-w-4xl text-5xl font-semibold leading-tight tracking-[-0.04em] text-slate-950 md:text-6xl">
                Book a local boat ride.
              </h1>
              <p className="max-w-2xl text-base leading-7 text-slate-600 md:text-lg">
                Choose the available trip, see where it goes, and pay when you arrive. Send a request
                and the captain will confirm the ride.
              </p>
          ...
          <div className="rounded-[32px] border border-white/70 bg-white/80 p-6 shadow-[0_20px_80px_rgba(15,23,42,0.12)] backdrop-blur">
            ...
            <div className="mt-5 space-y-4">
              <label className="block">
                <span className="mb-2 block text-sm font-medium text-slate-700">Where do you want to go?</span>
                <input
                  value={filters.query}
                  onChange={(event) =>
              ...
              <label className="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                <div>
                  <span className="block text-sm font-medium text-slate-800">Bags or supplies</span>
                  <span className="text-sm text-slate-500">Show trips that can carry more than passengers.</span>
                </div>
                <input
                  type="checkbox"
              ...
              <label className="block">
                <span className="mb-2 block text-sm font-medium text-slate-700">Seats needed</span>
                <input
                  type="range"
                  min={0}
                  max={8}
        ...
        <div className="flex items-center justify-between gap-4 rounded-[28px] border border-teal-100 bg-teal-50/70 px-5 py-4 text-sm text-teal-950">
          <p>Trips update when captains add new times.</p>
          <p className="shrink-0 rounded-full bg-white px-3 py-1 font-medium text-teal-700">
            {activeFilterCount} choice{activeFilterCount === 1 ? "" : "s"} set
          </p>
        </div>
            ...
            .onSuccess((boats: ReadonlyArray<BoatListingSummary>) => (
              <section className="space-y-5">
                <div className="flex items-end justify-between gap-4">
                  <div>
                    <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Available trips</p>
                ...
                {boats.length === 0 ? (
                  <div className="rounded-[28px] border border-dashed border-slate-300 bg-white/80 p-10 text-center text-sm text-slate-500">
                    No boats match right now. Try fewer seats or clear the search.
                  </div>
                ) : null}
              </section>
            ))
